...
public class KitakuTravelvsStartTime {
	...
	public static void main (String args[]) throws NumberFormatException, IOException, ParseException{
		...
		File out2 = new File ("c:/users/yabetaka/desktop/"+mode+"/kitakutraveltimes-starttime.csv");

		createMap(offices,filepath,homes, out2); //this is the MAIN Map for Kitaku times

		//		HashMap<Integer,Double> HODistance = HomeOfficeDistance.getDistance(homes, offices);

		//		File out = new File ("c:/users/yabetaka/desktop/"+mode+"/kitakutraveltimes-starttime.csv");
		//		writeout(map,21,out);
	}
	...
	public static File createMap(File offices,String path, File homes, File out) throws IOException, NumberFormatException, ParseException{
		HashMap<Integer, HashMap<Integer, Integer>> result = new HashMap<Integer, HashMap<Integer, Integer>>(); //<hour,<traveltime,starttime>>
		...
		for(int i=10; i<=22; i++){
			...
			else if(((i==13)||(i==12)||(i==5)||(i==6)||(i==19)||(i==20))){
				judge = 2;
			}
			...
			for(Integer id: id_firstHomelogofday.keySet()){
				if(id_lastOfficelogofday.containsKey(id)){
					int traveltime = id_firstHomelogofday.get(id)-id_lastOfficelogofday.get(id);
					if(judge == 0){
						if(count % 6 ==0){
							bw.write(judge + "," + traveltime + "," + id_lastOfficelogofday.get(id));
							bw.newLine();
						}
						count++;
					}
					...
					//						result.get(i).put(staytime, id_lastOfficelogofday.get(id));
					//					}
					//					else{
					//						HashMap<Integer, Integer> map = new HashMap<Integer, Integer>();
					//						map.put(staytime, id_lastOfficelogofday.get(id));
					//						result.put(i, map);
					//					}
				}
			}
	...
	public static File writeout(HashMap<Integer, HashMap<Integer, Integer>> map, int day, File out) throws IOException{
		...
		for (Integer d:map.keySet()){
			if(map.get(d).size()>=5){
				for (Integer kitakutraveltime : map.get(d).keySet()){
					if(d==day){
						bw.write(String.valueOf(kitakutraveltime + "," +map.get(d).get(kitakutraveltime)) + "," + 1);
						bw.newLine();
					}
					...
					//				}
						...
						count++;
						if(count%6==0){
							bw.write(String.valueOf(kitakutraveltime + "," + map.get(d).get(kitakutraveltime)) + "," + 0);
							bw.newLine();
						}
					}
				}
