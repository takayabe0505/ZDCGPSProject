...
public class StayPoint_KitakuAnalyser {

	public static void main (String args[]) throws NumberFormatException, IOException, ParseException{

		for(int i=10; i<=22; i++){
			String day = String.format("%02d", i);
			File in = new File ("c:/users/yabetaka/desktop/TyphoonGPS/FrequentStaypointswithTime.csv");
			File offices = new File ("c:/users/yabetaka/desktop/TyphoonGPS/id_OfficeSchool.csv");
			File TargetDayLogs = new File ("c:/users/yabetaka/desktop/TyphoonGPS/DatabyDays/DataforExp_"+day+".csv");

			HashMap<Integer,ArrayList<String>> id_lastlogs = intoResMap(in,offices,day); // get last logs of office data
			HashMap<Integer,ArrayList<Date>> id_DisData = sortTargetDayTime(TargetDayLogs,offices);
			HashMap<Integer,Integer> id_DislastlogTime = getDisExitTime(id_DisData);

			HashMap<Integer,Double> results = new HashMap<Integer, Double>();

			File out = new File("c:/users/yabetaka/desktop/likelihood/Kitaku/Kitakulikelihood_"+day+".csv");

			int count2 = 0;
//			System.out.println("size of DisData is: " +id_DisData.size());
			int count = 0;

			for(int id : id_lastlogs.keySet()){
				if(id_DislastlogTime.containsKey(id)){
					int dissec = id_DislastlogTime.get(id);
					double sum = 0d;
					double average = 0d;
					for(String time : id_lastlogs.get(id)){
...
//		int count = 0;
		for(Integer id : datamap.keySet()){
			...
			ArrayList<String> lastlogs = getLastLogofDay(officetimes);
...
//				count++;
	...
	public static ArrayList<String> getLastLogofDay(ArrayList<Date> alllogs) throws ParseException{
	...
	public static HashMap<Integer,ArrayList<Date>> sortTargetDayTime(File dislogs, File offices) throws IOException, ParseException{
	...
	public static HashMap<Integer,Integer> getDisExitTime(HashMap<Integer,ArrayList<Date>> map) throws ParseException{
